...
interface VoucherData {
  ...
  profileCode: string;
  samePass?: boolean;
  showPassword?: boolean; // untuk source=users, tampilkan password asli
}

function PrintContent() {
  const searchParams = useSearchParams();
  const key = searchParams.get('key');
  const dataStr = searchParams.get('data'); // fallback backward compat
  ...
  useEffect(() => {
    ...
    try {
      if (key) {
        // Coba baca dari sessionStorage
        const stored = sessionStorage.getItem(key);
        if (stored) {
          parsed = JSON.parse(stored);
        }
      } else if (dataStr) {
        // Fallback: baca dari URL param (backward compat untuk batch kecil)
  ...
  const cols = layout === '4x' ? 4 : layout === '2x' ? 2 : layout === 'thermal' ? 1 : 3;
  const qrSize = cols >= 4 ? 44 : cols === 3 ? 52 : 64;
  const isUsersSource = source === 'users';

  if (!loaded) {
    return <div style={{ padding: '2rem', textAlign: 'center', fontFamily: 'sans-serif' }}>Memuat data...</div>;
  }

  if (loaded && vouchers.length === 0) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center', fontFamily: 'sans-serif' }}>
        <p style={{ color: '#ef4444', fontWeight: 600 }}>⚠️ Tidak ada data voucher.</p>
        <p style={{ color: '#6b7280', marginTop: '0.5rem', fontSize: '0.875rem' }}>
          Data mungkin sudah kadaluarsa. Silakan kembali dan coba cetak ulang.
        </p>
    ...
    <div className="print-page">
      ...
      <div className="no-print print-controls">
        <div className="print-controls-inner">
          <div>
            <h2 style={{ margin: 0 }}>
              {isUsersSource ? 'Preview Cetak Daftar User' : 'Preview Cetak Voucher'}
            </h2>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {(['2x', '3x', '4x'] as const).map(l => (
              <button
                key={l}
                className={`btn btn-sm ${layout === l ? 'btn-primary' : 'btn-secondary'}`}
                onClick={() => {
              ...
              <div className="vc-body">
                <div className="vc-creds">
                  {isSame && !isUsersSource ? (
                    ...
                    <>
                      <div className="vc-field">
                        <span className="vc-label">Username</span>
                        <span className="vc-value">{v.username}</span>
                      </div>
                      <div className="vc-field">
                        <span className="vc-label">Password</span>
                        <span className="vc-value">{v.password}</span>
                      </div>
                    </>
                  )}
                ...
                {ssid && (
                  <div className="vc-qr">
                    <QRCodeSVG
                      value={`WIFI:T:WPA;S:${ssid};P:${v.password};;`}
                      size={qrSize}
                      level="M"
                      style={{ display: 'block' }}
                    />
      ...
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn {
          ...
          border: 1px solid transparent; text-decoration: none; transition: all 0.15s;
        }
        ...
        .vc-header {
          ...
          padding: 3px 7px; border-bottom: 1px dashed #e5e7eb;
        }
        .vc-biz {
          font-weight: 700; font-size: 0.7rem; color: #1e3a5f;
          white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 70%;
        }
        .vc-tag {
          font-size: 0.6rem; color: #6b7280; text-transform: uppercase;
          letter-spacing: 0.05em; background: #f3f4f6;
        ...
        .vc-value-lg {
          font-family: 'JetBrains Mono', monospace;
          font-size: 0.8rem; font-weight: 700; color: #1e3a5f; letter-spacing: 0.04em;
        }
        .vc-meta { display: flex; gap: 8px; margin-top: 2px; font-size: 0.55rem; color: #6b7280; }
        .vc-qr { flex-shrink: 0; display: flex; flex-direction: column; align-items: center; }
        .vc-footer {
          display: flex; justify-content: space-between;
          padding: 2px 7px; border-top: 1px dashed #e5e7eb;
